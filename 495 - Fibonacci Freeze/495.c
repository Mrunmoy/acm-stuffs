...
int add(int fn,int fn_1,int fn_2,char ch);

int main()
{
    int fib; //  0 <= n <= 5000
    int i,j,initial,final,temp_fib,found;
        ...
        else // other cases cacluate fib(n)
        {
            ...
            else
            {
                ...
                else // neither f(n-1) nor f(n-2) are available so calculate in normal iterative way
                {
                    ...
                    found = 0;
                    temp_fib=0;
                    // search in lookup table for faster execution time
                    for(i=initial;(i<=final) && (found!=1) && (initial <=final);i++)
                    {
                        temp_fib = (final+initial)/2;
                        found = LookUpTable.done[temp_fib]; // search in lookup table for already computed values.
                        if(found==1) // n/2 is already done so search further between n/2 and n
                        {
                            initial = temp_fib;
                            /*
                            if(found && !newfound)
                            {
                                this_is_not_the_last = 1;
                                for(;this_is_not_the_last;)
                                {
                                    if(LookUpTable.done[temp_fib]==0)
                                    {
                                        this_is_not_the_last = 0;
                                        temp_fib--;
                                    }
                                    else
                                    {
                        ...
                        else // n/2 is not done so search further between 0 and n/2
                        {
                            final = temp_fib;
                        }
                    }
                    if(found==1)
                    {
                        // start building up fibonacci numbers from temp_fib till fib
                        for(i=temp_fib+1;i<=fib;i++)
                        {
                            add(i,i-1,i-2,'n');
                            //printf("found!\n");
                            LookUpTable.done[i] = 1;
